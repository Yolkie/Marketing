...
app.put('/api/settings', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (facebook_app_id !== undefined) {
      await pool.query(
        'UPDATE settings SET value = $1, updated_by = $2, updated_at = CURRENT_TIMESTAMP WHERE key = $3',
        [facebook_app_id || '', req.user.userId, 'facebook_app_id']
      );
    }
    ...
    if (facebook_app_secret !== undefined) {
      await pool.query(
        'UPDATE settings SET value = $1, updated_by = $2, updated_at = CURRENT_TIMESTAMP WHERE key = $3',
        [facebook_app_secret || '', req.user.userId, 'facebook_app_secret']
      );
    }

    if (facebook_access_token !== undefined) {
      await pool.query(
        'UPDATE settings SET value = $1, updated_by = $2, updated_at = CURRENT_TIMESTAMP WHERE key = $3',
        [facebook_access_token || '', req.user.userId, 'facebook_access_token']
      );
    }
    ...
    if (facebook_page_id !== undefined) {
      await pool.query(
        'UPDATE settings SET value = $1, updated_by = $2, updated_at = CURRENT_TIMESTAMP WHERE key = $3',
        [facebook_page_id || '', req.user.userId, 'facebook_page_id']
      );
    }

    // Return updated settings
    const result = await pool.query(
      'SELECT key, value, description, updated_at FROM settings WHERE key IN ($1, $2, $3, $4, $5, $6, $7)',
      ['google_drive_folder_id', 'google_drive_api_key', 'n8n_recaption_webhook_url', 'facebook_app_id', 'facebook_app_secret', 'facebook_access_token', 'facebook_page_id']
    );
...
// ==================== FACEBOOK METRICS ROUTES ====================

// Get Facebook metrics for a content item
app.get('/api/facebook/metrics/:contentItemId', apiLimiter, authenticateToken, validateUUIDParam('contentItemId'), async (req, res) => {
  try {
    const { contentItemId } = req.params;

    const result = await pool.query(
      `SELECT * FROM facebook_metrics WHERE content_item_id = $1 ORDER BY updated_at DESC LIMIT 1`,
      [contentItemId]
    );
    ...
    console.error('Get Facebook metrics error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Get all Facebook metrics for published content
app.get('/api/facebook/metrics', apiLimiter, authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT 
        fm.*,
      ...
      JOIN content_items ci ON fm.content_item_id = ci.id
    ...
    console.error('Get all Facebook metrics error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Sync Facebook metrics from API
app.post('/api/facebook/sync/:contentItemId', apiLimiter, authenticateToken, validateUUIDParam('contentItemId'), async (req, res) => {
  try {
    const { contentItemId } = req.params;
    const { postId } = req.body;

    if (!postId) {
      return res.status(400).json({ error: 'Facebook post ID is required' });
    }
    ...
    if (!settings.facebook_access_token) {
      return res.status(400).json({ error: 'Facebook access token not configured. Please configure it in Settings.' });
    }

    const accessToken = settings.facebook_access_token;
    const pageId = settings.facebook_page_id || postId.split('_')[0]; // Use page ID from settings or extract from post ID

    // Fetch metrics from Facebook Graph API - using essential metrics fields
    const metricsFields = [
      'impressions',
      'reach',
      'post_engagements',
    ...
    const fbApiUrl = `https://graph.facebook.com/v18.0/${postId}/insights?fields=${metricsFields}&access_token=${accessToken}`;
    
    console.log('📡 Fetching Facebook metrics from API:', postId);
    ...
    if (!fbResponse.ok) {
      ...
      return res.status(fbResponse.status).json({ 
        error: 'Failed to fetch metrics from Facebook API',
    ...
    // Parse metrics from Facebook API response
    const metrics = {
      reach: 0,
      impressions: 0,
      engagements: 0,
    ...
    // Extract metrics from Facebook API response
    if (fbData.data && Array.isArray(fbData.data)) {
      fbData.data.forEach(metric => {
        const name = metric.name;
        const value = metric.values && metric.values[0] ? parseInt(metric.values[0].value) || 0 : 0;

        if (name === 'reach') metrics.reach = value;
        if (name === 'impressions') metrics.impressions = value;
        if (name === 'post_engagements') metrics.engagements = value;
        if (name === 'post_reactions_like_total') metrics.likes = value;
        if (name === 'comments') metrics.comments = value;
        if (name === 'shares') metrics.shares = value;
        if (name === 'post_clicks') metrics.clicks = value;
        if (name === 'post_video_views') metrics.video_views = value;
        if (name === 'link_clicks') metrics.link_clicks = value;
        
        // Calculate total reactions
        if (name.includes('post_reactions_')) {
          metrics.reactions += value;
        }
      });
    }

    // Calculate total engagements if not provided
    if (metrics.engagements === 0) {
      metrics.engagements = metrics.likes + metrics.comments + metrics.shares + metrics.clicks;
    }

    // Store or update metrics in database
    const existingResult = await pool.query(
      'SELECT id FROM facebook_metrics WHERE content_item_id = $1',
      [contentItemId]
    );
    ...
    if (existingResult.rows.length > 0) {
      ...
      await pool.query(
        `INSERT INTO facebook_metrics 
         (content_item_id, post_id, reach, impressions, engagements, likes, comments, shares, clicks, video_views, link_clicks, reactions, metrics_data, last_synced_at)
    ...
    const updatedResult = await pool.query(
      'SELECT * FROM facebook_metrics WHERE content_item_id = $1',
      [contentItemId]
    );

    res.json({ 
      message: 'Metrics synced successfully',
      metrics: updatedResult.rows[0]
    });
  } catch (error) {
